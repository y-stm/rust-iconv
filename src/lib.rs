...
#[derive(Debug)]
pub enum IconvError {
    OnFindingConversion(self::errno::Errno),
    ...
    OnClose(self::errno::Errno),
}

impl IconvError {
    pub fn iconv_err(e: self::errno::Errno) -> Self {
...
pub struct ToUTF8Conv {
    inner: Iconv,
}

impl ToUTF8Conv {
    pub fn new(encoding_from: &str) -> Result<Self, IconvError> {
        Iconv::new("utf-8", encoding_from).and_then(|iconv| Ok(ToUTF8Conv { inner: iconv }))
    }

    pub fn convert(&mut self, src: &[u8]) -> Result<String, IconvError> {
        let mut buf = Vec::new();
        self.inner
            .convert(src, &mut buf, 0)
            .and_then(|_| Ok(String::from_utf8(buf).unwrap()))
    }
}
#[test]
fn test_iconv_raw() {
...
pub fn test_convert(text: &[u8], encoding: &str) -> String {
